...
export const expenseReport = async (req: Request, res: Response) => {
  try {
    ...
    if (if_pdf_download) {
      ...
      const htmlContent = await ejs.renderFile(ejsFilePath, { expenses });
      ...
      totalCount = await expenseRepository.count(filter);
      ...
      if (page && limit) {
        ...
        expenses = await expenseRepository.find(filter);
      }

      return sendResponse(res, 200, 'Expense records retrieved successfully', {
        expenses,
        totalCount,
        totalNoOfRecords,
      });
